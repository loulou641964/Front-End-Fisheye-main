...
function updateLightbox() {
  ...
  document.querySelector("#divMediaLightbox").innerHTML =
    mediaHTML +
    `<p class="lightbox-paragraphe">${element.alt ?? element.title}</p>`;
}

// Fonction pour afficher le média précédent
function showPreviousMedia() {
  currentMediaIndex =
    currentMediaIndex > 0 ? currentMediaIndex - 1 : galleryElements.length - 1;
  updateLightbox();
}
...
function showNextMedia() {
  currentMediaIndex =
    currentMediaIndex < galleryElements.length - 1 ? currentMediaIndex + 1 : 0;
  updateLightbox();
}
...
function focusElement() {
  const focusableElements = document
    .querySelector("#divMediaLightbox")
